...
public class BobController {
	...
	public void update(float delta) {
		...
		checkCollisionWithPerks(delta);
		checkCollisionWithMashrooms(delta);
	...
	private void checkCollisionWithBlocks(float delta) {
		...
		for (Block block : collidable) {
			...
			if (bobRect.overlaps(block.getBounds())) {
				if(block.SUPER == 1 && world.getBob().SUPERPOWER == 1 ){
					world.getLevel().deleteWood((int)block.getPosition().y, (int)block.getPosition().x);
					continue;
				}else{
					bob.getVelocity().x = 0;
	...
	/** Collision checking **/
	private void checkCollisionWithPerks(float delta) {
		for(int i = 0; i < world.getLevel().getPerkArr().size; i++){
	...
	private void checkCollisionWithMashrooms(float delta) {
		for(int i = 0; i < world.getLevel().getMashroomArr().size; i++){
			if(world.getLevel().getMashroomArr().get(i).getPosition().x == Math.floor(bob.getPosition().x) &&
					world.getLevel().getMashroomArr().get(i).getPosition().y == Math.floor(bob.getPosition().y)){
				
				world.getLevel().getMashroomArr().get(i).useFeature(world.getBob());
